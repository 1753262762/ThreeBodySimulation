...
- 分支：`main`。
- 交付性质：Simulation Health 与可解释参数引导闭环的 WIP 基线。
- 当前本地服务：`http://127.0.0.1:8723`；该进程仅用于本次本地验收，换机或重启后需要重新启动。
- 设计与实施上下文：仓库根目录的 `SIMULATION_HEALTH_PROGRESS.md`、`GUIDED_PARAMETER_EXPERIENCE.md`、`GUIDED_PARAMETER_IMPLEMENTATION_PLAN.md`、`GUIDED_PARAMETER_PROGRESS.md`。

### 已完成能力

- 后端持久化权威 `SimulationHealthReport`，统一能量漂移、角动量漂移、peak、趋势、近遇关联、失败上下文和时间步长建议。
- REST 详情、报告、列表摘要以及 latest-wins `HEALTH` WebSocket 消息已接通；前端不再自行维护数值健康阈值。
- 参数校验返回权威配置摘要与结构化引导，包括预计步数、覆盖时间、结束条件、最近天体对、风险证据、建议动作与代价。
- 参数编辑器采用常驻短说明和按需展开解释；合法输入停止 500 ms 后自动校验，创建实验前再次校验并丢弃过期响应。
- 数值建议只自动调整时间步长及保持覆盖时间所需的最大步数，不自动改动物理初始条件或软化长度。
- Health 降级时可以创建新的对照实验；来源、重试深度、配置差异和源 Health 快照由服务端校验并持久化，源运行记录不会被覆盖。
- Health、事件面板和报告使用“结论—证据—验证方法—技术细节”结构，并区分数值失败与星系解体等物理现象。
- OpenAPI、WebSocket Schema、示例、生成 TypeScript 类型和离线 mock 已同步。

### 当前精确验证进度

- `mvn -pl simulation-web -am test`：Core 28、Application 77、Web 15，共 120 项通过。
- 前端定向 Vitest：14/14 通过。
- `npm run build`：TypeScript 检查和 Vite 生产构建通过；保留既有 3D chunk 大于 500 kB 警告。
- 完整前端单测：151/153 通过；两项未通过源于 jsdom 缺少 Canvas/WebGL 环境。
- Playwright E2E：11/12 通过；剩余 tooltip 用例假设固定画布中心必定命中天体。
- `mvn verify`：Core、Application、Web、Swing 通过；Launcher 执行 `npm ci` 时遇到 Windows `spawn EPERM`。随后已恢复依赖并单独完成前端生产构建。
- REST 冒烟：首页、实验列表和结构化配置校验通过。
- `git diff --check` 通过。
- lint 未运行：项目未提供 lint 命令。

### 已知限制与下一入口

- 先修正或重写两项 Canvas/WebGL 单测环境，以及 tooltip E2E 的命中前提，再追求完整前端测试全绿。
- 在允许创建子进程的 Windows 环境重新执行 `mvn clean verify`，确认 Launcher 的 `npm ci` 与最终 JAR 打包。
- 对一组固定物理配置进行人工对照运行，核对 GOOD/WARNING/POOR/FAILED 文案、对照实验来源关系以及报告展示。
- 本轮没有引入 Experiment→Run 父子模型；一个 `Experiment.id` 仍代表一次运行，历史对照通过创建新 ID 保留。
- 旧 `RESTART` API 仅为兼容保留，Health 引导流程不得调用它。
